...
u8 func_index = _Main_UI;                          // 当前页面索引值
    ...
    if (current_key.is_GuiCallback != 1)
    {
        ...
        if (func_index == _Card_Icron || func_index == _Setting_Icron || func_index == _Info_Icron)
        {
            ...
            case KEY_IN: //(current_key.Key_Type==KEY_SINGLE_CLICK)?func_index=MainMenu_Table[func_index].enter:func_index=MainMenu_Table[func_index].back;
                if (current_key.Key_Type == KEY_SINGLE_CLICK)
                    func_index = MainMenu_Table[func_index].enter;
                else if (current_key.Key_Type == KEY_LONG_CLICK)
                    func_index = MainMenu_Table[func_index].back;
                break;
            case KEY_RIGHT:
                func_index = MainMenu_Table[func_index].next; // 更新索引值
                break;
            case KEY_LEFT:
                func_index = MainMenu_Table[func_index].previous; // 更新索引值
                break;
            case KEY_DOWN:
                func_index = MainMenu_Table[func_index].next; // 更新索引值
                break;
...
{
    if (Popup_Sta.is_exist)
        Popup_Sta.callback_time++;
    else
    {
...
void Show_List(menuList_t *menuList_Table, uint8_t cur_pos, uint8_t max_item, uint8_t len)
{
    if (cur_pos <= max_item)
    {
        ...
        {
            if (cur_pos == i + 1)
                OLED_ShowStr(0, 2 * i + 1, (unsigned char *)menuList_Table[i].str, SIZE_0806, REVERSE);
            else
                OLED_ShowStr(0, 2 * i + 1, (unsigned char *)menuList_Table[i].str, SIZE_0806, NORMAL);
        }
    }
    else
    {
        ...
        {
            if (tem - i == max_item - 1)
                OLED_ShowStr(0, 2 * (tem - i) + 1, (unsigned char *)menuList_Table[cur_pos - i].str, SIZE_0806, REVERSE);
            else
                OLED_ShowStr(0, 2 * (tem - i) + 1, (unsigned char *)menuList_Table[cur_pos - i].str, SIZE_0806, NORMAL);
        }
    }
}
// 界面函数
...
{
    if (menu_last_index != _Card_Icron && menu_last_index != _Setting_Icron && menu_last_index != _Info_Icron)
        OLED_Clear();
...
void Card_Option_Func(u8 *page_index, u8 *key_val)
{
    ...
    if (menu_last_index == _Card_Option) // 判断是否是第一次进入此界面
    {
        ...
        case KEY_UP:
            if (head2tail)
                cur_pos == 1 ? cur_pos = option_num : cur_pos--;
            else
                cur_pos == 1 ? cur_pos = 1 : cur_pos--;
            break;
        ...
        case KEY_DOWN:
            if (head2tail)
                cur_pos == option_num ? cur_pos = 1 : cur_pos++;
            else
                cur_pos == option_num ? cur_pos = option_num : cur_pos++;
            break;
        default:
            break;
        }
...
void Add_Card_Func(u8 *page_index, u8 *key_val)
{
    ...
    Creat_Popup(_Card_Option, _Popup, _Popup, _Popup, AddCard_Popup, 10000);
}
...
void Delete_Card_Func(u8 *page_index, u8 *key_val)
{
    ...
    if (menu_last_index == _Delete_Card) // 判断是否是第一次进入此界面
    {
        ...
        {
        case KEY_UP:
            cur_pos == 1 ? cur_pos = 1 : cur_pos--; // cur_pos==1?cur_pos=nfc_uid.len:cur_pos--;
            break;
        case KEY_IN: // 确定(设置)按键
        {
            NFC_Event.Delete_Card = ON;
            ...
            NFC_Event.Delete_Card = OFF;
            // 刷新显存列表
            for (u8 i = 0; i < nfc_uid.len; i++)
                free(menuList_Table[i].str);
            free(menuList_Table);
            if (cur_pos > nfc_uid.len)
                cur_pos = nfc_uid.len;
            if (nfc_uid.len == 0)
        ...
        case KEY_DOWN:
            cur_pos == nfc_uid.len ? cur_pos = nfc_uid.len : cur_pos++; // cur_pos==nfc_uid.len?cur_pos=1:cur_pos++;
            break;
        default:
            break;
        }
